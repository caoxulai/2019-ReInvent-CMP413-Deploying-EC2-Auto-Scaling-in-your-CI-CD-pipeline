...
<body class="">
    ...
    <?php
      ...
      {
        print "Server name = <i>" . $hostname . "</i><br />";
      }
      // Get the ami ID of the instance from the instance metadata
      ...
      {
        print "Sorry, for some reason, we got no instance type back <br />";
      }
      else
      {
        print "EC2 Instance Type = <i>" . $instanceType . "</i><br />";
      }
      ...
      {
        print "EC2 Instance ID = <i>" . $instanceid . "</i><br />";
      }
      // Get the instance-type of the instance from the instance metadata
      curl_setopt($curl_handle,CURLOPT_URL,'http://169.254.169.254/latest/meta-data/ami-id');
      $amiId = curl_exec($curl_handle);
      if (empty($amiId))
      {
        print "Sorry, for some reason, we got no ami ID back <br />";
      }
      else
      {
        print "Amazon Machine Image ID = <i>" . $amiId . "</i><br /><br />";
      }
    ?>
            <a class="twitter-link" href="https://twitter.com/search?q=%23autoscaling%20%23aws&src=typd&lang=en"><img src="img/tweet.svg" /></a>
            <div class="text">
                <h1>Congratulations!</h2>
                <h2>You just deployed a web application running on<br/>EC2 Auto Scaling</h3>
                <br /><br />
            </div>
        </div>
    </div>
